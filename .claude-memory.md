# Claude Memory - Toolbox

Este arquivo serve como índice central da "caixa de ferramentas" - arquivos de memória persistente que definem workflows, processos e contextos para auxiliar nas interações com Claude neste diretório.

**Leia este arquivo no início de cada sessão para ter acesso às ferramentas e contextos disponíveis.**

---

## About

Este diretório contém arquivos de workflow e memória que permitem ao Claude manter consistência e eficiência nas interações. Cada arquivo de memória documenta um processo específico que pode ser invocado durante as sessões.

---

## 🌐 Cloud Memory Integration (NEW - v2.3)

**Status**: ✅ Infrastructure ready
**Repository**: https://github.com/3x-Projetos/claude-memory-cloud (PRIVATE)
**Location**: `~/.claude-memory-cloud/`

### What is Cloud Memory?

**Multi-device sync for your AI interactions**:
- 🔄 Same memories on all devices (laptop, desktop, phone)
- 🧠 Continue work seamlessly across devices
- 📊 Unified timeline of all activities
- 🔒 Private GitHub repo for security

### How to Set Up

**On each device:**
```bash
# Option 1: Automated (recommended)
bash .claude/setup-cloud-memory.sh

# Option 2: Manual
git clone https://github.com/3x-Projetos/claude-memory-cloud.git ~/.claude-memory-cloud
```

### Documentation

- **Integration Guide**: `.claude/CLOUD-MEMORY-INTEGRATION.md` ⭐ READ THIS FIRST
- **Setup Script**: `.claude/setup-cloud-memory.sh`
- **Population Script**: `.claude/populate-cloud-memory.sh`
- **Cloud Memory Repo**: `~/.claude-memory-cloud/README.md` (after cloning)

### Quick Sync

```bash
# Pull latest (before starting work)
cd ~/.claude-memory-cloud && git pull

# Push changes (after ending session)
cd ~/.claude-memory-cloud && git add . && git commit -m "Update" && git push
```

**Future**: Automatic sync via enhanced hooks (coming in v2.3)

---

## Ferramentas Disponíveis

### 1. Workflow de Organização de Notas
**Arquivo**: `.workflow-claude-notes-organization.md`

**Propósito**: Define o processo padrão para organização de notas diárias com identificação de tópicos e sugestão de prioridades.

**Quando usar**: Sempre que houver um arquivo de notas com blocos `[raw]` e `[prompt]` que precise ser processado.

**Estrutura**:
- Blocos: `[raw]` → `[prompt]` → `[organized]`
- Separadores: `---` entre blocos
- Tags sempre em minúsculas

**Comando típico**:
```
leia o [arquivo].md e execute a ação contida no bloco [prompt].
Registre o retorno no local [organized], mantendo a tag.
```

**Output esperado**:
- Topics Identified & Organized
- Suggested Priorities com próximos passos

### 2. Workflow de Session Logging
**Arquivo**: `.workflow-session-logging.md`

**Propósito**: Registrar todas as interações e atividades realizadas em cada sessão para manter histórico e continuidade.

**Quando usar**:
- Início: `/start` carrega contexto e lembra de fazer log
- Fim: `/end` cria/atualiza log da sessão

**Estrutura**:
- Arquivo: `YYYY.MM.DD.md` (exemplo: `2025.11.15.md`)
- Bloco: `[session-log]`
- Seções: Agente, Atividades, Arquivos modificados, Próximos passos, Notas

**Obrigatório**: Toda sessão deve ter seu log registrado

**Multi-Agent Support**: Campo "Agente" identifica qual AI trabalhou na sessão (Claude, Gemini, etc)

### 3. Workflow de Session Continuity
**Arquivo**: `.workflow-session-continuity.md`

**Propósito**: Manter contexto entre sessões, permitindo retomar trabalho de onde parou ou iniciar nova atividade com awareness do histórico.

**Como funciona**:
- `/start` lê automaticamente o log mais recente
- Apresenta resumo da última sessão
- Lista pendências (tarefas não concluídas)
- Pergunta se deve continuar ou iniciar novo foco

**Benefícios**:
- Continuidade natural sem reexplicar contexto
- Retoma direto de onde parou
- Flexibilidade para mudar de foco mantendo histórico

---

## Convenções Gerais

### Nomenclatura de Arquivos de Memória
- Prefixo com `.` para arquivos ocultos de workflow
- Formato: `.workflow-[nome-descritivo].md`
- Memória central: `.claude-memory.md` (este arquivo)

### Tags e Marcadores
- Sempre minúsculas: `[raw]`, `[prompt]`, `[organized]`, etc.
- Separadores visuais: `---`
- Blocos claramente demarcados

---

## Frameworks de Priorização

### Quick Win Framework
**Uso**: Priorizar tarefas baseado em benefício vs dificuldade

**Critérios**:
- **Benefício**: Impacto/ganho da tarefa (Alto/Médio/Baixo)
- **Dificuldade**: Tempo e complexidade de implementação (Alta/Média/Baixa)

**Matriz de Priorização**:
1. 🎯 **Quick Wins** (Alto benefício + Baixa dificuldade) → PRIORIDADE MÁXIMA
2. 📈 **Major Projects** (Alto benefício + Alta dificuldade) → Planejar bem
3. 🔧 **Fill Ins** (Baixo benefício + Baixa dificuldade) → Quando sobrar tempo
4. ❌ **Time Wasters** (Baixo benefício + Alta dificuldade) → Evitar

**Exemplo aplicado**:
- SessionEnd hook: Alto benefício (nunca perde sessão) + Baixa dificuldade (5-10 min) = 🎯 Quick Win

**Quando usar**: Ao planejar implementações ou decidir próximos passos

---

## Slash Commands Disponíveis

### `/continue`
**Uso recomendado para retomar trabalho**. Carrega ferramentas e contexto da última sessão, assume que vai continuar de onde parou. Apresenta pendências e continua direto.

### `/new`
**Uso recomendado para nova atividade**. Carrega ferramentas e awareness da última sessão, mas foca em começar algo novo. Não lista pendências.

### `/start`
Carrega ferramentas e contexto, mas **pergunta** se deseja continuar ou iniciar novo. Use quando quiser escolher na hora.

### `/memory`
Consulta a caixa de ferramentas durante a sessão. Útil para relembrar workflows disponíveis.

### `/organize`
Executa o workflow de organização de notas. Pode especificar o arquivo ou será perguntado.

### `/end`
Finaliza a sessão criando/atualizando o log de atividades do dia (YYYY.MM.DD.md). Pergunta sobre atividades realizadas e cria registro estruturado.

### `/auto-approve`
**Toggle em tempo real de auto-aprovação com múltiplos níveis.**

Uso:
- `/auto-approve on` - Ativa modo "edits" (Edit/Write apenas)
- `/auto-approve bash` - Ativa modo "bash" (Edit/Write/Bash)
- `/auto-approve all` - Ativa modo "all" (todos os tools)
- `/auto-approve off` - Desativa tudo (comportamento padrão)
- `/auto-approve status` - Verifica nível atual

Controla quais operações requerem aprovação manual. Mudanças aplicam instantaneamente.

**Níveis disponíveis**:
- `off` - Nada aprovado automaticamente
- `edits` - Apenas Edit e Write
- `bash` - Edit, Write e Bash (padrão recomendado para fluxo fluido)
- `all` - Todos os tools (máxima automação)

---

## Auto-Aprovação com Múltiplos Níveis

**Status**: ✅ Configurado com toggle em tempo real e múltiplos níveis

**Arquivos**:
- `.claude/settings.json` - Configuração local do hook
- `.claude/auto-approve-state` - Arquivo de estado (off/edits/bash/all)
- `.claude/auto-approve-edits.py` - Script PreToolUse com lógica de níveis
- `.claude/AUTO-APPROVE-README.md` - Documentação completa

**Propósito**: Elimina prompts de aprovação manual para operações selecionadas, com controle granular.

**Níveis disponíveis**:
- `off` - Nada aprovado automaticamente
- `edits` - Apenas Edit e Write
- `bash` - Edit, Write e Bash (padrão recomendado para fluxo fluido)
- `all` - Todos os tools (máxima automação)

**Como usar**:
1. **Primeira vez**: Reinicie o Claude CLI para registrar o hook
2. **Padrão recomendado**: `bash` (já configurado)
   - Aprova: Edit, Write, Bash
   - Fluxo de trabalho fluido sem interrupções
3. **Outros níveis disponíveis**:
   - `/auto-approve edits` → apenas Edit/Write
   - `/auto-approve all` → todos os tools (máxima automação)
   - `/auto-approve off` → desativa tudo
4. **Verificar**: Use `/auto-approve status` para ver nível atual

**Toggle em tempo real**: Hook lê arquivo de estado a cada operação, permitindo mudar de nível sem reiniciar CLI.

**Escopo**: Funciona APENAS neste diretório (não afeta outros projetos).

**Importante**: Configuração LOCAL (`.claude/settings.json`), não global. Portável e isolada por projeto.

---

## Graceful Shutdown (Lazy Logging Strategy)

**Status**: ✅ Implementado com estratégia Lazy Logging

**Propósito**: Garantir que **nenhuma sessão seja perdida**, mesmo se você encerrar com Ctrl+C sem usar `/end`.

**Estratégia - "Lazy Logging"**:
Registra a sessão anterior no **próximo startup** (em vez de tentar registrar no shutdown).

**Como funciona**:

1. **SessionStart hook** (`.claude/session-start.py`):
   - Executa automaticamente ao iniciar CLI
   - **PASSO 1**: Verifica se existe `.previous-session-id`
     - Se SIM → sessão anterior não foi logged → **cria registro automático AGORA**
     - Se NÃO → sessão anterior foi logged via `/end` → nada a fazer
   - **PASSO 2**: Marca início da nova sessão (cria novo `.previous-session-id`)

2. **Durante a sessão**:
   - Se você usa `/end` → deleta `.previous-session-id` + cria log detalhado ✅
   - Se você não usa `/end` → arquivo `.previous-session-id` permanece

3. **Ao encerrar com Ctrl+C**:
   - CLI fecha imediatamente (sem executar hooks)
   - Arquivo `.previous-session-id` fica lá (será processado no próximo startup)

**Exemplo prático**:
```
Sessão 1:
  - Startup: Sem .previous-session-id → nada a fazer
  - Trabalho...
  - Encerra com Ctrl+C → .previous-session-id permanece

Sessão 2:
  - Startup: Detecta .previous-session-id da Sessão 1
  - Cria entrada automática: "Sessão 14:30 (Lazy Logging)"
  - Deleta .previous-session-id
  - Cria novo .previous-session-id para Sessão 2
  - Trabalho...
  - Usa /end → log detalhado + deleta .previous-session-id ✅

Sessão 3:
  - Startup: Sem .previous-session-id → Sessão 2 foi logged → nada a fazer
```

**Tipos de log**:
- **Com `/end`**: Log detalhado com atividades, métricas, próximos passos
- **Sem `/end`**: Log básico com apenas timestamp (placeholder)

**Arquivos**:
- `.claude/session-start.py` - Lazy Logging + marca início da nova sessão
- `.claude/.previous-session-id` - Estado da sessão (criado no startup, deletado pelo `/end`)
- `.claude/.current-session-id` - Mantido para compatibilidade
- `.claude/settings.json` - Hook SessionStart registrado

**Benefícios**:
- ✅ Funciona no Windows com Ctrl+C (SessionEnd não funciona)
- ✅ Zero fricção (funciona silenciosamente)
- ✅ Nunca perde sessão (sempre haverá registro, mesmo que básico)
- ✅ Suporta múltiplas sessões por dia
- ✅ Não interfere com `/end` manual
- ✅ Logs detalhados quando usa `/end`, placeholders quando esquece

**Limitação conhecida**: Logs automáticos não contêm conteúdo (apenas timestamp), pois CLI não tem acesso ao histórico de conversas após encerrar. **Use `/end` para logs completos.**

**Ativação**: Requer reiniciar Claude CLI uma vez. Após isso, funciona automaticamente em todas as sessões.

---

## Como Replicar Este Sistema em Novo Projeto

**Arquivo**: `.claude/setup-claude-memory.sh`

**Como usar**:
```bash
# Opção 1: Executar direto deste diretório
bash .claude/setup-claude-memory.sh /caminho/do/novo/projeto

# Opção 2: Copiar script e executar lá
cp .claude/setup-claude-memory.sh /novo/projeto/
cd /novo/projeto
bash setup-claude-memory.sh
```

**O que faz**:
- Cria toda estrutura (memória, workflows, comandos)
- Instala documentação completa
- Sistema fica idêntico ao deste diretório
- Pronto para usar após reiniciar Claude CLI

**Após instalação**:
1. Reiniciar Claude CLI
2. Usar `/start` para começar
3. Sistema completo funcionando

---

## Sistema de Memória Hierárquica

**Status**: ✅ Implementado (FASE 0-6 completas)
**Versão**: 2.0

### Arquitetura

Sistema completo de memória multi-camadas com agregação temporal para economia de tokens e personalização contínua.

**Memória Global** (multi-projeto):
- Localização: `~/.claude-memory/`
- Perfil do usuário versionado
- Compartilhado entre todos os projetos Claude
- PII protegido via redação automática

**Memória Local** (por projeto):
- Working memory: `.session-state.md` (~50 linhas)
- Logs diários: `logs/daily/YYYY.MM.DD.md` (detalhado)
- Resumos semanais: `logs/weekly/YYYY.MM.weekN.md` (~100 linhas)
- Resumos mensais: `logs/monthly/YYYY.MM.md` (~30 linhas)

**Economia de Tokens**: 50-96% dependendo da hierarquia carregada

### Estrutura de Diretórios

```
# Projeto Local
.session-state.md              # Working memory (ativo)
.projects/                     # Project-Centric Memory (M008)
│   ├── README.md
│   └── [project-name]/
│       ├── .context.md        # Working memory do projeto
│       ├── .status.md         # Roadmap, decisões, métricas
│       └── history/           # Logs específicos (opcional)
logs/
├── daily/                     # Logs detalhados
├── weekly/                    # Agregações semanais
└── monthly/                   # Agregações mensais
.claude/
└── performance/               # Performance Tracking (M009)
    ├── profiles/              # Performance profiles por modelo
    ├── comparisons/           # Análises comparativas mensais
    └── context-analysis/      # Insights de otimização

# Memória Global (~/.claude-memory/)
global-memory.md               # Perfil completo (com PII)
global-memory.safe.md          # Perfil redacted (auto-gerado)
profile-history/               # Snapshots versionados
│   └── YYYY-MM-DD_*.md
profile-changelog.md           # Histórico de mudanças
projects/                      # Referências a projetos
    └── [project-name].md
```

### Workflows de Agregação

**4 Workflows documentados**:
1. `.workflow-weekly-aggregation.md` - Dia → Semana (~85% economia)
2. `.workflow-monthly-aggregation.md` - Semana → Mês (~93% economia vs diários)
3. `.workflow-profile-update.md` - Atualização de perfil (mensal ou trigger-based)
4. `.workflow-metrics-collection.md` - Métricas holísticas (7 dimensões)

### Comandos Slash Atualizados

**9 Comandos disponíveis**:
- `/start` - Pergunta se continua ou inicia novo
- `/continue` - Retoma de onde parou (carrega hierarquia + pendências)
- `/new` - Nova atividade (carrega hierarquia, sem pendências)
- `/memory` - Consulta ferramentas disponíveis
- `/organize` - Organiza notas
- `/end` - Finaliza sessão (logs + métricas + session-state)
- `/update-profile` - Atualiza perfil global (manual)
- `/reflect` - Coleta métricas de well-being
- `/aggregate` - Força agregação semanal/mensal

### Sistema de Privacidade (PII Redaction)

**Arquivo**: `.claude/redact-pii.py`

**Funcionamento**:
- Marca PII em `global-memory.md`: `[PII:TYPE]value[/PII:TYPE]`
- Gera `global-memory.safe.md`: `[REDACTED:TYPE]`
- Executado automaticamente por `/continue` e `/new`

**Tipos PII suportados**:
- NAME, EMAIL, LOCATION, COMPANY, PROJECT
- CREDENTIAL, API, DOCUMENT

**Híbrido**:
- Dados completos localmente (`global-memory.md`)
- Dados redacted em transmissão (`.safe.md`)

### Framework de Métricas Holísticas

**Arquivo**: `.claude/METRICS-FRAMEWORK.md`

**7 Dimensões de Impacto Humano**:
1. Performance & Productivity
2. Cognitive Load & Mental Energy
3. Well-Being & Satisfaction
4. Learning & Growth
5. Autonomy & Empowerment
6. Collaboration Quality
7. Life Integration

**Coleta**:
- **Inferidos**: Análise automática de logs (complexity, velocity, tech exposure)
- **Calculados**: Derivados (time saved, autonomy growth)
- **Relatados**: Via `/reflect` (satisfaction, energy, frustration)

**Agregação**: Métricas aparecem em logs diários, resumos semanais e mensais

### Perfil Global Versionado

**Conteúdo**:
- User Profile (identity, working style, tech preferences, communication)
- Collaboration Patterns (como trabalhar com Claude)
- Projects Context (projetos ativos)
- Opportunities & Quick Wins
- Recurring Architecture Decisions
- Meta-Learnings

**Versionamento**:
- Semântico: Major.Minor (ex: 2.1)
- Snapshots em `profile-history/`
- Changelog completo em `profile-changelog.md`

**Atualização**:
- Mensal (automático via `/continue`)
- Threshold (3+ menções/semana de mudança)
- Manual (via `/update-profile`)

### Como Usar

**Início de sessão**:
```
/continue  → Retoma trabalho (pendências listadas)
/new       → Nova atividade (sem pressão de pendências)
```

**Durante sessão**:
```
/reflect   → Registra well-being (opcional)
/organize  → Processa notas
```

**Fim de sessão**:
```
/end       → Cria log + métricas + atualiza session-state
```

**Manutenção**:
```
/aggregate week      → Toda segunda-feira (recomendado)
/aggregate month     → Todo dia 1 do mês (recomendado)
/update-profile      → Quando detectar mudanças significativas
```

### Benefícios

**Economia de Tokens**:
- Carregamento típico: ~330 linhas (hierarquia)
- Vs logs brutos: ~5.000+ linhas
- **Economia: ~93%**

**Personalização Contínua**:
- Claude aprende suas preferências ao longo do tempo
- Adapta colaboração ao seu working style
- Sugere melhorias baseadas em padrões

**Insights Holísticos**:
- Tracking de impacto humano (além de produtividade)
- Tendências de well-being, aprendizado, autonomia
- Identificação de padrões e oportunidades

**Continuidade**:
- Zero perda de contexto entre sessões
- Retoma trabalho exatamente de onde parou
- Histórico completo rastreável

---

## Project-Centric Memory Layer (M008)

**Status**: ✅ Implementado
**Versão**: 2.1+

### Propósito

Sistema **bidimensional** de memória que suporta trabalho multi-projeto:
- **Dimensão temporal**: daily → weekly → monthly (já existente)
- **Dimensão projeto**: working memory + status por projeto (NOVO)

### Por Que Foi Criado

**Problema**: Framework anterior era unidimensional (time-centric apenas)
- Usuário trabalha em múltiplos projetos paralelamente
- Aumentava clutter mental
- Difícil rastrear status por projeto
- Context switching manual

**Solução**: Adicionar camada project-centric

### Estrutura

```
.projects/
├── README.md                  # Templates e categorias
└── [project-name]/
    ├── .context.md            # Working memory do projeto
    ├── .status.md             # Roadmap, milestones, decisões
    └── history/               # Logs específicos (opcional)
```

### Categorias Suportadas (Visão Holística)

- 💻 Code
- 🎨 Creative
- 🏗️ Physical
- 👤 Personal
- 🤝 Social/Community
- 💼 Business/Finance
- 🤖 AI/Research
- 🌐 Infrastructure/Network
- 🌍 Other

**Filosofia**: Sistema suporta todos os aspectos da vida, não apenas código

### Comandos Disponíveis

**Novos**:
- `/projects` - Dashboard de todos os projetos (agrupados por categoria)
- `/switch [project-name]` - Muda contexto para projeto específico
- `/project-status [project-name]` - Atualiza roadmap/decisões

**Atualizados**:
- `/continue` - Pergunta qual projeto trabalhar
- `/new` - Permite escolher projeto ou criar novo
- `/end` - Registra "projects touched" com % tempo por projeto

### Benefícios

- ✅ Redução de clutter mental (tudo organizado por projeto)
- ✅ Context switching fluido (memória específica por projeto)
- ✅ Tracking de status claro (COMPLETE/IMPLEMENTING/PLANNING/PAUSED/BLOCKED)
- ✅ Visão holística (não apenas código - todos os aspectos da vida)
- ✅ Dashboard consolidado (ver todos os projetos de uma vez)

---

## Agent Performance Tracking (M009)

**Status**: ✅ Estrutura implementada, coleta inicia no próximo `/end`
**Versão**: 2.1+

### Propósito

Metrificar e comparar performance de diferentes agentes e modelos para decisões **data-driven** sobre qual usar em cada contexto.

### Por Que Foi Criado

**Problema**:
- Sem dados sobre o que cada modelo/agente faz bem
- Escolha de modelo é empírica (não data-driven)
- Desperdício de tokens/custo usando modelo errado para tarefa
- Janela de contexto não é otimizada (prompts muito longos/curtos)
- Não há baseline para comparar modelos

**Solução**: Sistema de tracking contínuo com foco em **context window optimization**

### Estrutura

```
.claude/performance/
├── README.md
├── TEMPLATE-performance-profile.md
├── profiles/
│   ├── claude-sonnet-4.5.md
│   ├── claude-haiku.md
│   ├── gemini-1.5-pro.md
│   └── ...
├── comparisons/
│   └── 2025.11.md
└── context-analysis/
    └── prompt-optimization.md
```

### Métricas Coletadas

**Quantitativas (Auto)**:
- Tokens/session, Context window usage, Prompt length distribution
- Output/Input ratio, Latency, Cost
- Efficiency (framework vs sem), Volume (files, lines)
- Context limit hits

**Qualitativas (Inferidas + Opcionalmente Perguntadas)**:
- Task type, Specialization, Quality rating (1-5 stars)
- Success rate, Pattern detection, Context efficiency

**Context Window Metrics** (foco principal):
- Window utilization (avg, peak, limit hits)
- Optimal prompt range (correlação qualidade vs tamanho)
- Sweet spot identification (ex: 40-60% = melhor qualidade)
- Diminishing returns threshold
- Framework ROI (economia de tokens)

### Workflow de Coleta

**Integrado no `/end` (Passo 4)**:
1. **Auto-detectar**: modelo, tokens, duration, files, tech, task type, project
2. **Perguntar (opcional)**: qualidade (1-5), completou? (yes/partial/no), categoria
3. **Registrar**: append to performance profile
4. **No log diário**: incluir seção "Performance Metrics"

### Análises Geradas

**Mensal** (via `/aggregate month`):
- Re-calcular stats agregados
- Análise comparativa entre modelos
- Identificar sweet spots de contexto por task type
- Context optimization insights

**Insights Esperados** (exemplos):
```
📊 Context Analysis: Tarefas de architecture têm qualidade 4.8/5
quando contexto usado é 50-70%, mas cai para 4.1/5 quando >85%.

💡 Model Selection: Tarefa "refactor simples" detectada.
Considere Haiku (20x mais rápido, 75% mais barato, qualidade 4.2/5).
```

### Benefícios

- **Economia de tokens**: Escolher modelo right-sized
- **Melhor qualidade**: Usar modelo especializado quando importa
- **Prompt optimization**: Identificar comprimento ótimo
- **Context efficiency**: Saber quando mais contexto ajuda vs atrapalha
- **Comparative analysis**: Data para decidir qual modelo usar
- **ROI do framework**: Quantificar benefício da hierarquia
- **Multi-agent orchestration**: Dados para routing inteligente (futuro)

### Fases de Implementação

- ✅ Fase 1 (Implementada): Estrutura, templates, coleta integrada no `/end`
- 📋 Fase 2 (Após 2-3 semanas): Análise comparativa mensal
- 📋 Fase 3 (Após 1-2 meses): Recomendações automáticas no `/end`
- 📋 Fase 4 (Futuro): Multi-agent routing automático

---

## Como Adicionar Novas Ferramentas

1. Crie arquivo `.workflow-[nome].md` documentando o processo
2. Adicione entrada nesta seção "Ferramentas Disponíveis"
3. Descreva: arquivo, propósito, quando usar, estrutura esperada
4. Opcionalmente, crie slash command em `.claude/commands/[nome].md`

---

## Contexto do Diretório

**Tipo**: Obsidian Vault
**Uso**: Notas pessoais, organização de ideias, workflows de criação
**Tópicos principais**: Creative workflow, image generation, LLM infrastructure, scene development

---

*Última atualização: 2025-11-16*
